...
from networks.FFTUnet3D import FFTunet_3D
...
def net_factory_3d(net_type="unet_3D", in_chns=1, class_num=2):
    ...
    elif net_type == "FFTUnet3D":
        net = FFTunet_3D(n_channels=in_chns, n_classes=class_num,
                   normalization='batchnorm', has_dropout=True).cuda()
    elif net_type == "vnet2D":
        net = VNet2D(n_channels=in_chns, n_classes=class_num,
                   normalization='batchnorm', has_dropout=True).cuda()
    else:
        net = None
    return net
